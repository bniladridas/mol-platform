...
def _random_mutation(mol):
    ...
    mutation_strategies = [_add_random_substituent, _modify_bond_order, _swap_atom]
...
def _add_random_substituent(mol):
    ...
    substituents = ["O", "N", "C(=O)", "C(=N)", "Cl", "Br", "F", "S"]
        ...
        if atom.GetTotalNumHs() > 0:  # Ensure the atom can accept a substituent
            try:
                ...
                new_mol.ReplaceSubstructs(
                    Chem.MolFromSmarts("[#1:1]"), new_substituent, idx
                )
                return new_mol
            except Exception:
                continue
...
def _modify_bond_order(mol):
    ...
    for idx in bond_indices:
        ...
        try:
            new_mol = Chem.RWMol(mol)
            new_mol.GetBondWithIdx(idx).SetBondType(Chem.BondType(new_order))
            return new_mol
        except Exception:
            continue
...
def test_molecule_generation():
    ...
    for base_smiles in base_molecules:
        ...
        logger.info(
            f"Generated molecule from {base_smiles}: {result['generated_smiles']}"
        )
